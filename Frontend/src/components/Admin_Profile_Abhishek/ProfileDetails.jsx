...
const StartupProfileDetails = ({ id }) => {
		...
		if (id) {
			try {
				const response = await axios.get(
					`https://localhost:3007/api/StartupProfile/v1/${id}`,
					{
						headers: {
			...
			console.log(`https://localhost:3007/api/StartupProfile/v1/${id}`);
			console.log(data);
		}
	};
	useEffect(() => {
	...
	const handleReject = async () => {
		...
		try {
			await axios.patch(
				`https://localhost:3007/api/StartupProfile/u1/${id}`,
				{
	...
	const postNotification = async (notificationMessage, docLink = null, subtitle = "Startup Profile Application") => {
		try {
			...
			const response = await axios.post(
				"https://localhost:3007/api/notifications/",
				notificationData,
				{
					headers: {
						"Content-Type": "application/json",
