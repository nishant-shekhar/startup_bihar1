...
const StartupProfileDetails = ({ id }) => {
		...
		if (id) {
			try {
				const response = await axios.get(
					`http://51.20.148.118:3007/api/StartupProfile/v1/${id}`,
					{
						headers: {
			...
			console.log(`http://51.20.148.118:3007/api/StartupProfile/v1/${id}`);
			console.log(data);
		}
	};
	useEffect(() => {
	...
	const handleReject = async () => {
		...
		try {
			await axios.patch(
				`http://51.20.148.118:3007/api/StartupProfile/u1/${id}`,
				{
	...
	const postNotification = async (notificationMessage, docLink = null, subtitle = "Startup Profile Application") => {
		try {
			...
			const response = await axios.post(
				"http://51.20.148.118:3007/api/notifications/",
				notificationData,
				{
					headers: {
						"Content-Type": "application/json",
