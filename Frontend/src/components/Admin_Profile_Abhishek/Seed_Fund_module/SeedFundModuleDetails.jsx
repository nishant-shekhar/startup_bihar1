...
const SeedfundModuleDetails = ({ id }) => {
	...
	const fetchData = async () => {
		if (id) {
			try {
				const response = await axios.get(`http://localhost:3007/api/seed-fund/v1/${id}`, {
					headers: {
	...
	const getComment = () => {
		...
		if (data.documentStatus === "Accepted") return "Document has been reviewed and approved.";
		if (data.documentStatus === "Rejected") return "Document has been partially rejected";
		if (data.documentStatus === "Partially Rejected") return "Document has been rejected";
		return "";
	};
						...
						<tr>
							...
							{data.companyCertificate && (
                                    <div>
                                        ...
                                        >
                                            View
                                        </button> | <a className="text-blue-500 hover:underline" href={`${data.companyCertificate}`} download>Download</a>
                                    </div>
                                )}
							</td>
						</tr>
						...
						<tr>
							...
							{data.cancelChequeOrPassbook && (
                                    <div>
                                        ...
                                        >
                                            View
                                        </button> | <a className="text-blue-500 hover:underline" href={`${data.cancelChequeOrPassbook}`} download>Download</a>
                                    </div>
                                )}
							</td>
						</tr>
