...
const IPRReimbursementModule = ({ id }) => {
		...
		if (id) {
			try {
				const response = await axios.get(
					`https://startupbihar.in/api/iprReimbursement/v1/${id}`,
					{
						headers: {
	...
	const handleReject = async () => {
		...
		try {
			await axios.patch(
				`https://startupbihar.in/api/iprReimbursement/u1/${id}`,
				{
	...
	const handlePartialReject = async () => {
		...
		try {
			...
			const docLinks = selectedOptions.map(option => {
				const docName = option === "iprCertificateFilePath" ? "Project Report" :
					option === "consultancyInvoiceFilePath" ? "GST Return" :
						option === "feePaidInvoiceFilePath" ? "Audited Balance Sheet" : option;
				...
				<table className="min-w-full bg-white">
					<tbody>
						...
						<tr>
							...
							<td className="py-4 px-4 border">{data.iprType}</td>
						</tr>
            <tr>
							<td className="py-4 px-4 border">
              Fee Paid For Application Form
							</td>
							<td className="py-4 px-4 border">{data.feePaidForApplicationForm}</td>
						</tr>
            <tr>
							<td className="py-4 px-4 border">
              Consultancy Fee
							</td>
							<td className="py-4 px-4 border">{data.consultancyFee}</td>
						</tr>
					
						<tr>
							<td className="py-4 px-4 border">
              IPR Certificate File 
							</td>
										...
										<ul className="divide-y divide-gray-100 rounded-md border border-gray-200">
											<li className="flex items-center justify-between py-4 pl-4 pr-5 text-sm/6">
												...
												<div className="ml-4 shrink-0">
													<button
														onClick={() => handleViewPdf(data.iprCertificateFilePath)}
												...
												<div className="ml-4 shrink-0">
													<a
														href={data.iprCertificateFilePath} // Ensure it's used as an absolute URL
										...
										<ul className="divide-y divide-gray-100 rounded-md border border-gray-200">
											<li className="flex items-center justify-between py-4 pl-4 pr-5 text-sm/6">
												...
												<div className="ml-4 shrink-0">
													<button
														onClick={() =>
															handleViewPdf(data.feePaidInvoiceFilePath)
														}
														className="font-medium text-indigo-600 hover:text-indigo-900"
													>
														View
												...
												<div className="ml-4 shrink-0">
													<a
														href={data.feePaidInvoiceFilePath}
														download
														className="font-medium text-indigo-600 hover:text-indigo-900"
													>
														Download
										...
										<ul className="divide-y divide-gray-100 rounded-md border border-gray-200">
											<li className="flex items-center justify-between py-4 pl-4 pr-5 text-sm/6">
												<div className="flex w-0 flex-1 items-center">
													...
													<div className="ml-4 flex min-w-0 flex-1 gap-2">
														<span className="truncate font-medium">
															{data.consultancyInvoiceFilePath ? (
																data.consultancyInvoiceFilePath
															) : (
																<span className="text-red-500">
																	File is either rejected or not available
												...
												<div className="ml-4 shrink-0">
													<button
														onClick={() => handleViewPdf(data.consultancyInvoiceFilePath)}
														className="font-medium text-indigo-600 hover:text-indigo-900"
													>
														View
													</button>
												...
												<div className="ml-4 shrink-0">
													<a
														href={data.consultancyInvoiceFilePath} // Ensure it's used as an absolute URL
														download
														className="font-medium text-indigo-600 hover:text-indigo-900"
													>
														Download
