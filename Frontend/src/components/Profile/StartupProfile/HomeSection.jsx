	...
	const fetchEmployeeDetails = async () => {
		try {
			...
			const response = await axios.get(
				`http://51.20.148.118:3007/api/userlogin/getEmployees?startupId=${localStorage.getItem("user_id")}`,
			);
	...
	const fetchDetails = async () => {
		try {
			...
			const showcaseResponse = await axios.get(
				`http://51.20.148.118:3007/api/showcase/get-showcase/${localStorage.getItem("user_id")}`
			);
						...
						<div className="relative bg-white bg-opacity-80 backdrop-filter backdrop-blur-lg border border-white border-opacity-30 w-5/12 p-8 rounded-lg shadow-lg">
							...
							<Formik
								...
								onSubmit={(values, { setSubmitting }) => {
									...
									axios
										.post("http://51.20.148.118:3007/api/showcase/post", formData, {
											headers: {
