	...
	const fetchDetails = async () => {
		try {
			const response = await axios.get(
				`https://startupbihar.in:3007/api/userlogin/startup-details?user_id=${id}`,
			);
			...
			const showcaseResponse = await axios.get(
				`https://startupbihar.in:3007/api/showcase/get-showcase/${id}`
			);
			setShowcases(showcaseResponse.data.showcase);

			const employeeResponse = await axios.get(`https://startupbihar.in:3007/api/userlogin/getEmployees?startupId=${id}`);
